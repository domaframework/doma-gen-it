...
/** @author nakamura-to */
public class GenItGlobalFactory extends GlobalFactory {
  ...
  public DataSource createDataSource(Driver driver, String user, String password, String url) {
    ...
    try {
      ...
      try {
        ...
        statement.executeUpdate(
            "create table category(id integer not null primary key, name varchar(20), version integer)");
        statement.executeUpdate(
            "create table product(id integer not null primary key, primary_category_id integer not null, secondary_category_id integer not null ,version integer, constraint product_fk1 foreign key(primary_category_id) references category(id), constraint product_fk2 foreign key(secondary_category_id) references category(id))");
        statement.executeUpdate(
            "create table product2(id integer not null primary key, category integer not null, version integer, constraint product2_fk1 foreign key(category) references category(id))");
        statement.executeUpdate(
            "create table integer_table (integer_column integer, version integer)");
        statement.executeUpdate("create table int_table (int_column int, version integer)");
        statement.executeUpdate(
            "create table double_table (double_column double, version integer)");
        statement.executeUpdate(
            "create table double_precision_table (double_precision_column double precision, version integer)");
        statement.executeUpdate("create table float_table (float_column float, version integer)");
        statement.executeUpdate(
            "create table varchar_table (varchar_column varchar(20), version integer)");
        statement.executeUpdate(
            "create table varchar_ignorecase_table (varchar_ignorecase_column varchar_ignorecase, version integer)");
        statement.executeUpdate("create table char_table (char_column char, version integer)");
        statement.executeUpdate(
            "create table character_table (character_column character, version integer)");
        statement.executeUpdate(
            "create table longvarchar_table (longvarchar_column longvarchar, version integer)");
        statement.executeUpdate("create table date_table (date_column date, version integer)");
        statement.executeUpdate("create table time_table (time_column time, version integer)");
        statement.executeUpdate(
            "create table timestamp_table (timestamp_column timestamp, version integer)");
        statement.executeUpdate(
            "create table datetime_table (datetime_column datetime, version integer)");
        statement.executeUpdate(
            "create table decimal_table (decimal_column decimal, version integer)");
        statement.executeUpdate(
            "create table numeric_table (numeric_column numeric, version integer)");
        statement.executeUpdate(
            "create table boolean_table (boolean_column boolean, version integer)");
        statement.executeUpdate("create table bit_table (bit_column bit, version integer)");
        statement.executeUpdate(
            "create table tinyint_table (tinyint_column tinyint, version integer)");
        statement.executeUpdate(
            "create table smallint_table (smallint_column smallint, version integer)");
        statement.executeUpdate(
            "create table bigint_table (bigint_column bigint, version integer)");
        statement.executeUpdate("create table real_table (real_column real, version integer)");
        statement.executeUpdate(
            "create table varbinary_table (varbinary_column varbinary(100), version integer)");
        statement.executeUpdate(
            "create table longvarbinary_table (longvarbinary_column longvarbinary, version integer)");
        statement.executeUpdate("create view employeeview as select * from employee");
